# This is a version of an original BMIQ script, stylistically modified for readability.
# Added random seed setting as well
#
# -----------------------
#
### BMIQ.R & CheckBMIQ.R
### This function adjusts for the type2 bias in Illumina Infinium 450k data. CheckBMIQ generates plot to check normalisation.
### Author: Andrew Teschendorff
### Date: 4th Oct 2012

### INPUT:
### beta.v: vector consisting of beta-values
### design.v: vector specifying probe design type (1=type1,2=type2).
### doH: perform normalisation for hemimethylated type2 probes. By default TRUE.
### nfit: number of probes of a given design to use for the fitting. Default is 50000. Smaller values will make BMIQ faster at the expense of accuracy.
...
BMIQ <- function(beta.v,design.v,nL=3,doH=TRUE,nfit=50000,th1.v=c(0.2,0.75),th2.v=NULL,niter=5,tol=0.001,plots=F,sampleID=1, seed=100){
    require(RPMM);
    set.seed(seed)
    ...
    ### check if there are spurious zeros (replace by minimum non-zero)
    if(min(beta1.v)==0) {
        zero.idx <- which(beta1.v==0);
        beta1.v[zero.idx] <- min(setdiff(beta1.v,0));
    }
    if(min(beta2.v)==0){
        zero.idx <- which(beta2.v==0);
        beta2.v[zero.idx] <- min(setdiff(beta2.v,0));
    }
    ...
    print("Done");
    ### generate from estimated mixture
#    if(plots){
#        print("Check");
...
#    }
    ...
    #pU <- length(which(beta2.v < th2.v[1]))/length(beta2.v);
    #pM <- length(which(beta2.v > th2.v[2]))/length(beta2.v);
    #prior2.v <- c(pU,1-pU-pM,pM);
...
#    }
    ...
    #p.v <- pbeta(beta2.v[selML.idx],em2.o$a[lt],em2.o$b[lt],lower.tail=TRUE);
    ### find corresponding quantile in type I distribution
    #q.v <- qbeta(p.v,em1.o$a[lt],em1.o$b[lt],lower.tail=TRUE);
    #nbeta2.v[selML.idx] <- q.v;
    ...
    print("Finished for sample");
    return(list(all=pnbeta.v,class1=class1.v,class2=class2.v,av1=classAV1.v,av2=classAV2.v,hf=hf,th1=nth1.v,th2=th2.v));
...
CheckBMIQ <- function(beta.v,design.v,pnbeta.v){### pnbeta is BMIQ normalised profile
    ...
    legend(x=0.5,y=ymax,legend=c("type1","type2","type2-NORM"),bty="n",fill=c("red","black","blue"));
}
